...
@Service
public class StudenteService extends GenericService<Long, // TipoID
...
> {
    ...
    // metodi che permetteono di avere i servizi necessari per gestire uno
    // studente(CRUD + relazioni)
    // in formato DTO
    ...
    // deleteStudente senza cancellarlo ma inserendo come valore del suo status
    // "INATTIVO"
    ...
    // metodo per cercare uno studente tramite id e aver caricate già le sue
    // iscrizioni(se ce ne sono)
    ...
    public boolean cambiaPassword(Long studenteId, String vecchiaPassword, String nuovaPassword) {
        // Controlliamo se lo studente esiste
        Studente s = getByIdOrNull(studenteId);
        if (s == null)
            return false;

        return userAccountService.updatePassword(s, vecchiaPassword, nuovaPassword);
    }
}
